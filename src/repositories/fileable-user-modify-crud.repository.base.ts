...
export abstract class FileableUserModifyCrudRepository<T extends FileableUserModifiableEntity, ID, Relations extends object = {}> extends DefaultUserModifyCrudRepository<T, ID, Relations> {
  ...
  async addFiles(fileableId: FileableUserModifiableEntity | String, files: (File | String) | (File | String)[]): Promise<FileableFile | FileableFile[]> {
    let fileId: String;
    ...
    for (let file of files) {
      ...
      fileableFiles.push(new FileableFile({
  ...
  async deleteFiles(fileableId: FileableUserModifiableEntity | String, files: File | String | (File | String)[]): Promise<Count> {
    let fileId;
    let orFilter = [];
    if (!(fileableId instanceof String) && fileableId.id) fileableId = fileableId.id
  ...
  async create(entity: (DataObject<T> & {fileObjects?: FileObject[], deleteFileObjects?: string[]}), options?: Options): Promise<T> {
    let files: File[] = []
    ...
    let res = await super.create(entity, options);
    await this.fileableRepo.create(res);
    await this.addFiles(res, files);
    return res;
  }

  async createAll(entities: (DataObject<T> & {fileObjects?: FileObject[], deleteFileObjects?: string[]})[], options?: Options): Promise<T[]> {
    let res: T[] = [];
  ...
  async update(entity: T & {fileObjects?: FileObject[], deleteFileObjects?: string[]}, options?: Options): Promise<void> {
    let files: File[] = []
    if (entity.fileObjects) files = await this.handleFileObjects(entity.fileObjects)
    delete entity.fileObjects
  ...
  async updateById(id: ID, data: DataObject<T> & {fileObjects?: FileObject[], deleteFileObjects?: string[]}, options?: Options): Promise<void> {
    let files: File[] = []
  ...
  private async handleFileObjects(fileObjects: FileObject[]): Promise<File[]> {
    ...
    for (const fileObject of fileObjects) {
      fileRecord = await this.fileRepo.findOne({where: {fileId: fileObject.fileId}})
      if (!fileRecord?.id) {
      ...
      this.fileRepo.update(fileRecord)
      // upload File
      if (!fileRecord.blobSync) this.blobstorageService.uploadFile(fileRecord, fileRecord.tenantId)
  ...
  async findOne(filter?: Filter<T>, options?: Options): Promise<(T & Relations) | null> {
    const filters = this.splitInclusionFilter(filter)
    if (filters) {
      let resp = await super.findOne(filters?.fileableFilter, options)
  ...
  async findById(id: ID, filter?: Filter<T>, options?: Options): Promise<T & Relations> {
    const filters = this.splitInclusionFilter(filter)
    if (filters) {
      let resp = await super.findById(id, filters?.fileableFilter, options)
      return this.includeFilesOne(resp, filters?.fileScopeFilter)
    }
  ...
  private async includeFiles(fileables: (T & Relations)[], filter?: Filter<AnyObject>): Promise<(T & Relations)[]> {
    ...
    //create filter
    let filesFilter = {where: {or: orFilter}, include: [{relation: 'file', scope: filter}]}
    let fileablesFilesRes = await this.fileableFileRepo.findAll(filesFilter)

    //insert all fileablesFiles into object for better finding
    let map: {[fileableId: string]: FileableFile[]} = {}
    for (const fileableFile of fileablesFilesRes) {
